...
每个工厂封装一个通用飞书操作（列日程、建任务、查文档……），把工具描述、`name`、是否需要审批、
是否以用户身份执行、所需授权范围，以及少数接口真实需要的本地化参数等注入，并产出一个
[feishu.agent.tools.Tool][]。处理函数经
...
from ._base import reauth_on_permission_error
