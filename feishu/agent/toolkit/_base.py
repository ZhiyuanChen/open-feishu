...
async def list_recent_payment_accounts(*, approval_code: str | None = None, limit: int = 10) -> list[Any]:
    r"""返回请求用户本人的收款账户句柄（脱敏标签），按轮上下文未配置账户解析器或无法确定用户时返回空列表。"""
    context = current_tool_context()
...
async def resolve_payment_account(account_id: str) -> dict[str, Any] | ToolResult:
    r"""
    把一个收款账户句柄解析为可提交的账户控件值，严格按请求用户隔离；不可用时返回 `BLOCKED` 结果。

    账户句柄到完整值的唯一入口：请求用户取自 [feishu.agent.context.ToolContext.requesting_user][]（绝不可由
    模型指定），解析委托给按轮上下文中的 [feishu.agent.payment_accounts.PaymentAccountResolver][]。未配置解析器、
    无法确定请求用户、或句柄不属于该用户 / 已失效时返回 `BLOCKED` 的 [feishu.agent.result.ToolResult][]，调用方
    应直接返回它；否则返回完整账户值。模型只传句柄，永远拿不到完整卡号。
    """
    context = current_tool_context()
    resolver = context.payment_accounts
    if resolver is None:
